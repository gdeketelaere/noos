...
$symtomes	    = doctreat_get_taxonomy_array('symtomes');
		...
		<div class="dc-dashboardbox">
			...
			<?php 
				while ($query->have_posts()) : $query->the_post();
					...
					$db_sym				= apply_filters('doctreat_get_tax_query',array(),$article_id,'symtomes','');
					$db_lan				= apply_filters('doctreat_get_tax_query',array(),$article_id,'languages','');
					$description 		= $post_content;
					$db_cat				= !empty( $db_cat ) ? wp_list_pluck($db_cat,'term_id') : array();
					$db_tag				= !empty( $db_tag ) ? wp_list_pluck($db_tag,'term_id') : array();
					$db_sym				= !empty( $db_sym ) ? wp_list_pluck($db_sym,'term_id') : array();
					$db_lan				= !empty( $db_lan ) ? wp_list_pluck($db_lan,'term_id') : array();
				...
				<form class="dc-post-artical" method="post">
					<div class="dc-dashboardboxcontent dc-addservices dc-articlesservices">
						...
						<div class="dc-category-holder dc-tabsinfo">
							<div class="dc-tabscontenttitle">
								<h3><?php esc_html_e('Select Symtomes','doctreat');?></h3>
							</div>
							<div class="dc-articletag-holder">
								<div class="dc-formtheme dc-skillsform">
									<fieldset>
										<div class="form-group">
											<div class="form-group-holder">
												<select class="dc-chosen-select" data-placeholder="<?php esc_attr_e('Choose Symtomes','doctreat');?>" name="post_symtomes[]" multiple>
													<?php 
														if( !empty( $symtomes ) ){							
															foreach ($symtomes as $key => $item) {
																$term_id   = $item->term_id;
																$selected = '';
																if( in_array($term_id,$db_sym) ){
																	$selected = 'selected';
																}
																?>
																	<option <?php echo esc_attr( $selected );?> value="<?php echo esc_attr( $term_id ); ?>"><?php echo esc_html( $item->name ); ?></option>
