...
if( !function_exists(  'doctreat_get_verification_check' ) ) {
	...
	function doctreat_get_verification_check($user_id='',$text='Verified user'){
		
		if( !empty($user_id) ) {
			$is_verified 	= get_post_meta($user_id, '_is_verified', true);
			ob_start();
			if( !empty( $is_verified ) && $is_verified === 'yes' ){ ?>
				<i class="far fa-check-circle dc-awardtooltip dc-tipso" data-tipso="<?php esc_attr_e($text,'doctreat');?>"></i>
			<?php } 
				echo ob_get_clean();
			}
	}
...
if( !function_exists( 'doctreat_post_date' ) ){
    function doctreat_post_date( $post_id = '' ){              
		...
		if( !empty($post_date) ) {?>
			<span class="dc-datetime"><i class="lnr lnr-clock"></i> <?php echo date($date_formate,strtotime($post_date));?></span>
		<?php 
		}
        echo ob_get_clean();       
    }
	...
	function doctreat_get_doctor_thumnail($profile_id='' ,$width = 100 , $height = 100){
		...
		if( !empty( $profile_id ) ) {
			...
			<figure class="dc-docpostimg">
				<img class="dc-image-res" src="<?php echo esc_url( $avatar_url );?>" alt="<?php echo esc_attr( $display_name );?>">
				<img class="dc-image-res-2x" src="<?php echo esc_url( $avatar_2x );?>" alt="<?php echo esc_attr( $display_name );?>">
					...
					<span>
						<i class="lnr lnr-clock"></i><?php 
							...
							foreach( $bookig_days as $val ){ 
								...
								if( $val == $day ){  
									$availability	= 'yes';
									echo '<em class="dc-dayon">'.$day_name.'</em>'; 
								} else {
...
if( !function_exists(  'doctreat_get_doctor_booking' ) ) {
	...
	function doctreat_get_doctor_booking( $profile_id='',$show_btn='yes'){
		...
		if( !empty( $profile_id ) ) {
			...
			$profile_url	= get_the_permalink( $profile_id );
			$profile_url	= !empty( $profile_url ) ? $profile_url : '';
			$bookig_days	= doctreat_get_booking_days( $profile_id );
			$bookig_days	= !empty( $bookig_days ) ? $bookig_days : array();
...
if (!function_exists('doctreat_get_name_bases')) {
    ...
    function doctreat_get_name_bases($key='') {
       $name_bases = array(
		...
		$name_bases	= apply_filters('doctreat_filter_get_name_bases',$name_bases);
		if( !empty($key) ){
...
if( !function_exists(  'doctreat_get_group_services_with_speciality' ) ) {
	...
	function doctreat_get_group_services_with_speciality( $profile_id = '',$selected ='', $return = 'echo',$type=''){
		if( !empty($profile_id) ){
			if( empty($type) ){
	...
	add_action( 'doctreat_get_group_services_with_speciality', 'doctreat_get_group_services_with_speciality',10,4);
	add_filter( 'doctreat_get_group_services_with_speciality', 'doctreat_get_group_services_with_speciality',10,4);
}
