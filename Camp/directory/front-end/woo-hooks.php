    ...
    function doctreat_update_bookings_data( $order_id,$user ) {
		...
		if( !empty( $items ) ) {
			...
			if( !empty( $order_detail ) ){
				...
				$post_meta['_services']			= !empty( $order_detail['service'] ) ? $order_detail['service'] : array();
				$post_meta['_consultant_fee']	= !empty( $order_detail['consultant_fee'] ) ? $order_detail['consultant_fee'] : 0;
				$post_meta['_appointment_date']	= !empty( $order_detail['appointment_date'] ) ? $order_detail['appointment_date'] : '';
...
if (!function_exists('doctreat_get_booking_payment_title')) {
    ...
    function doctreat_get_booking_payment_title($key) {
		
		$bookings	= doctreat_get_booking_payment();
...
if (!function_exists('doctreat_get_booking_payment')) {
    ...
    function doctreat_get_booking_payment() {
		$booking	= array(
				'myself' 		=> array('title' => esc_html__('MySelf','doctreat')),
				...
				'price' 			=> array('title' => esc_html__('Total Price','doctreat')),
						...
						<?php 
							...
							?>
								<?php if( $key === 'service' ) { ?>
									...
									<?php } else { ?>
										<div class="cart-style"> 
											<span class="style-lable"><?php echo doctreat_get_booking_payment_title( $key );?></span> 
											<span class="style-name"><?php echo doctreat_get_booking_value( $value,$key );?></span> 
										</div>
									<?php }?>
							<?php }?>
					  </div>
											...
											if( !empty($payment_type) && $payment_type === 'bookings') {?>
												...
												<?php } else { ?>
													<div class="cus-options-data">
														...
														<div class="step-value"><?php echo doctreat_get_booking_value( $value,$key );?></div> 
													</div>
												<?php }?>
											<?php } else if( !empty($payment_type) && $payment_type === 'subscription' ) { ?>
												<div class="cus-options-data">
