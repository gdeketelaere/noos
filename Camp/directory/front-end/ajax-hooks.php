...
if( !function_exists( 'doctreat_update_appointment' ) ){
    function doctreat_update_appointment(){
		...
		$required	= array(
							'post_id' 		=> esc_html__('Post ID is required.','doctreat'),
							'start_time' 	=> esc_html__('Start time is required.','doctreat'),
							'end_time' 		=> esc_html__('End time is required.','doctreat'),
							'intervals' 	=> esc_html__('Interval is required.','doctreat'),
							'spaces' 		=> esc_html__('Check Apointment Spaces.','doctreat'),
...
if( !function_exists( 'doctreat_add_article' ) ){
    function doctreat_add_article(){       
		...
		$post_symtomes	    = !empty( $_POST['post_symtomes'] ) ? $_POST['post_symtomes'] : array(0);
		$post_languages	    = !empty( $_POST['post_languages'] ) ? $_POST['post_languages'] : array(0);
		...
		if( empty( $post_title ) ){
			...
			if( $post_id ) {
				...
				wp_set_object_terms( $post_id, $post_symtomes, 'symtomes' );
				wp_set_object_terms( $post_id, $post_languages, 'languages' );
				$json['type']    = 'success';
 ...
 * @return 
 */
if( !function_exists( 'doctreat_update_doctor' ) ){
    function doctreat_update_doctor(){       
        global $current_user,$theme_settings;               
        $json = array();
		...
		$am_starting_price  	= !empty($_POST['am_starting_price'] ) ? sanitize_text_field($_POST['am_starting_price']) : '';
    ...
    add_action('wp_ajax_doctreat_update_doctor_profile', 'doctreat_update_doctor');
    add_action('wp_ajax_nopriv_doctreat_update_doctor_profile', 'doctreat_update_doctor');
}
/**
...
if ( !function_exists( 'doctreat_follow_doctors' ) ) {
	...
	function doctreat_follow_doctors() {
		...
		if ( empty( $current_user->ID ) ) {
			$json['type'] = 'error';
			$json['message'] = esc_html__( 'You must login before add this to wishlist.', 'doctreat' );
			wp_send_json( $json );
		}
        ...
        if (!empty($post_id)) {
            if( in_array($post_id, $wishlist ) ){                
                $json['type'] 		= 'error';
                $json['message'] 	= esc_html__('This is already to your wishlist', 'doctreat');
                wp_send_json( $json );
            } else {
				$wishlist[] = $post_id;
				...
				$json['type'] 		= 'success';
				$json['message'] 	= esc_html__('Successfully! added to your wishlist', 'doctreat');
				wp_send_json( $json );
			}
        }
...
if ( !function_exists( 'doctreat_get_booking_data' ) ) {
	...
	function doctreat_get_booking_data() {
		...
		if(!empty( $post_id ) ){
			...
			$service_html			= apply_filters('doctreat_get_group_services_with_speciality',$post_id,'','return','location');
...
if ( !function_exists( 'doctreat_booking_doctor' ) ) {
	...
	function doctreat_booking_doctor() {
		...
		$required	= array(
			...
			'service' 			=> esc_html__( 'Please select the service(s)', 'doctreat' ),
...
if ( !function_exists( 'doctreat_booking_step1' ) ) {
	...
	function doctreat_booking_step1() {
		...
		if( !empty( $booking_hospitals ) && !empty( $booking_slot ) && !empty( $appointment_date )) {
			if( !empty( $myself ) && $myself === 'someelse' ) {
				$_SESSION['booking']['_other_name']		= $other_name;
				$_SESSION['booking']['_relation']		= $relation;
			}
			
			$_SESSION['booking']['post_title']				= get_the_title( $booking_hospitals );
			$_SESSION['booking']['post_content']			= $booking_content;
			$_SESSION['booking']['_booking_service']		= $booking_service;
			$_SESSION['booking']['_booking_slot']			= $booking_slot;
			$_SESSION['booking']['_booking_hospitals']		= $booking_hospitals;
			$_SESSION['booking']['_appointment_date']		= $appointment_date;
			$_SESSION['booking']['_doctor_id']				= $doctor_id;
			$_SESSION['booking']['_myself']					= $myself;
...
if ( !function_exists( 'doctreat_booking_resend_code' ) ) {
	...
	function doctreat_booking_resend_code() {
		...
		if( $current_user->ID ) {
			...
			$_SESSION['booking']['email']				= $current_user->user_email;
			$_SESSION['booking']['user_type']			= 'registered';
			$_SESSION['booking']['authentication_code']	= $key_hash;
	...
	function doctreat_booking_step2() {
		...
		if( $current_user->ID ) {
			...
			if( !empty( $email ) && !empty( $full_name ) && is_email($email) && !empty( $phone_number )) {
				...
				$_SESSION['booking']['user_type']			= 'guest';
				$_SESSION['booking']['full_name']			= $full_name;
				$_SESSION['booking']['phone_number']		= $phone_number;
				$_SESSION['booking']['authentication_code']	= $key_hash;
...
if ( !function_exists( 'doctreat_booking_step3' ) ) {
	...
	function doctreat_booking_step3() {
		...
		if( empty( $code ) ) {
			...
			if(isset( $_SESSION['booking']['authentication_code'] ) ) {
				
				if( trim( $_SESSION['booking']['authentication_code'] ) === trim( $code ) ) {
					$author_id 				= $current_user->ID;
					$services				= !empty( $_SESSION['booking']['_booking_service'] ) ? $_SESSION['booking']['_booking_service'] : array();
					$doctor_id				= !empty( $_SESSION['booking']['_doctor_id'] ) ? doctreat_get_linked_profile_id( $_SESSION['booking']['_doctor_id'] ) : '';
					
					$doct_hospital			= !empty( $_SESSION['booking']['_booking_hospitals'] ) ? $_SESSION['booking']['_booking_hospitals'] : '';
					$email					= !empty( $_SESSION['booking']['email'] ) ? $_SESSION['booking']['email'] : '';
					...
					$am_specialities 		= doctreat_get_post_meta( $doctor_id,'am_specialities');
					$am_specialities		= !empty( $am_specialities ) ? $am_specialities : array();
					$booking_slot			= !empty( $_SESSION['booking']['_booking_slot'] ) ? $_SESSION['booking']['_booking_slot'] : '';
					$appointment_date		= !empty( $_SESSION['booking']['_appointment_date'] ) ? $_SESSION['booking']['_appointment_date'] : '';
					$rand_val				= rand(1, 9999);
					...
					if(!empty($services)) {
						foreach( $services as $key => $vals ) {
							foreach( $vals as $k => $v ) {
								$total_price	= $price  + $am_specialities[$key][$v]['price'];
							}
						}
					}
					...
					if(!empty($payment_type) && $payment_type == 'offline' && empty($enable_checkout_page)){
						...
						$myself		= !empty( $_SESSION['booking']['_myself'] ) ? $_SESSION['booking']['_myself'] : '';
						$contents	= !empty( $_SESSION['booking']['post_content'] ) ? $_SESSION['booking']['post_content'] : '';
						$post_title		= !empty( $theme_settings['appointment_prefix'] ) ? $theme_settings['appointment_prefix'] : esc_html__('APP#','doctreat');
						$booking_post 	= array(
											'post_title'    => wp_strip_all_tags( $post_title ).'-'.$rand_val,
											'post_status'   => 'pending',
						...
						if(!empty($booking_id)){
							if( !empty( $myself ) && $myself === 'someelse' ) {
								$relation	= !empty( $_SESSION['booking']['_relation'] ) ? $_SESSION['booking']['_relation'] : '';
								$other_name	= !empty( $_SESSION['booking']['_other_name'] ) ? $_SESSION['booking']['_other_name'] : '';
						...
						$cart_meta['slots']		= !empty( $_SESSION['booking']['_booking_slot'] ) ?  $_SESSION['booking']['_booking_slot'] : '';
						$cart_meta['appointment_date']		= !empty( $_SESSION['booking']['_appointment_date'] ) ?  $_SESSION['booking']['_appointment_date'] : '';
						$cart_meta['hospital']	= $doct_hospital;
						$cart_meta['doctor_id']	= $doctor_id;
						$cart_meta['content']	= !empty( $_SESSION['booking']['post_content'] ) ?  $_SESSION['booking']['post_content'] : '';
						$cart_meta['myself']	= !empty( $_SESSION['booking']['_myself'] ) ?  $_SESSION['booking']['_myself'] : '';
						
						if( empty( $current_user->ID ) ) {
							$cart_meta['user_type']		= !empty( $_SESSION['booking']['user_type'] ) ?  $_SESSION['booking']['user_type'] : '';
							$cart_meta['full_name']		= !empty( $_SESSION['booking']['full_name'] ) ?  $_SESSION['booking']['full_name'] : '';
							$cart_meta['phone_number']	= !empty( $_SESSION['booking']['phone_number'] ) ?  $_SESSION['booking']['phone_number'] : '';
							$cart_meta['email']			= !empty( $_SESSION['booking']['email'] ) ?  $_SESSION['booking']['email'] : '';
						}
						
						if( !empty( $cart_meta['myself'] ) && $cart_meta['myself'] === 'someelse' ) {
							$cart_meta['other_name']	= !empty( $_SESSION['booking']['_other_name'] ) ?  $_SESSION['booking']['_other_name'] : '';
							$cart_meta['relation']		= !empty( $_SESSION['booking']['_relation'] ) ?  $_SESSION['booking']['_relation'] : '';
						}
	...
	function doctreat_get_booking_byID() {
		...
		if(!empty($booking_id)) {
			...
			<div class="dc-user-header">
				...
				<?php } ?>
				<div class="dc-title">
					<a href="javascript:;"><?php echo esc_html( ucfirst( $user_type ) );?></a>
					<?php if( !empty( $name ) ){?>
						<h3>
							<?php 
								echo esc_html( $name ); 
			...
			<div class="dc-user-details">
				<div class="dc-user-grid">
					...
					<?php if( !empty( $location_title ) ){?>
						<div class="dc-user-info">
							<div class="dc-title">
								<h4><?php esc_html_e('Apointment location','doctreat');?> :</h4>
					...
					<?php if( !empty( $appointment_date ) && !empty( $slots ) ){?>
						<div class="dc-user-info">
							<div class="dc-title">
								...
								<span><?php echo date( $date_format,strtotime( $appointment_date ) );?> - <?php echo esc_html($slots);?> </span>
							</div>
						</div>
					<?php } ?>
										...
										<div class="dc-spec-wrap">
											...
											<ul class="dc-required-details">
												<?php foreach( $sers as $k => $val) { ?>
													<li><span><?php echo doctreat_get_term_name( $k ,'services');?></span></li>
												<?php } ?>
											</ul>
											<?php } ?>
										</div>
