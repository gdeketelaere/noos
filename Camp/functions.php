...
*/
    ...
    function camp_customize($wp_customize)
    {
        ...
        $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'camp_api_google_map', array(
            'label'      => __('AIzaSyD0TxncR9sd4ZGGZAw---b8DDs8j4BVAKE'),
  ...
  PARTIE ENQUEU SCRIPTS ET STYLES : CHARGEMENT DES SCRIPTS ET STYLES PERSO
    ...
    function camp_enqueue_scripts()
    {
        ...
        //JS
        //wp_enqueue_script('camp-googleapi-js', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyD0TxncR9sd4ZGGZAw---b8DDs8j4BVAKE');
  ...
  PARTIE CPT : TYPE DE POSTS PERSO
    ...
    function camp_register_programme()
    {
         ...
         */
        /*$labels = array(
		"name" => __( "Hôpital", "camp" ),
		"singular_name" => __( "Hôpital", "camp" ),
	);

	$args = array(
		"label" => __( "Hôpital", "camp" ),
		"labels" => $labels,
		"description" => "",
		"public" => true,
		...
		"map_meta_cap" => true,
		"hierarchical" => false,
		"rewrite" => array( "slug" => "hospitals", "with_front" => true ),
		"query_var" => true,
...
add_action('admin_menu', 'camp_disable_new_posts');*/
...

	{
		...
		add_post_meta($linked_profile, '_href_see_more', 1);
            ...
            function camp_delete_location()
            {
                ...
                if (!is_admin()) {
                    if (
                        isset($_POST['camp_location_nonce']) &&
  ...
  PARTIE FORMULAIRE MULTICRITERES
            ...
            function camp_get_favorit_check($post_id = '', $size = '')
            {
                ...
                <a href="javascript:;" class="<?php echo esc_attr($class); ?> dc-btn" data-id="<?php echo intval($post_id); ?>"><?php _e('Enregistrer', 'doctreat'); ?></a>
        <?php }
                    echo ob_get_clean();
                }
            }
        ...
        <style>
            ...
            .dc-deleteinfo .dashicons-trash,
            .remove-repeater .dashicons-trash {
                color: red;
            }
        </style>
        <!-- script-->
        ...
        <script type="text/javascript">
            function fetch() {
                ...
                jQuery.ajax({
                    ...
                    type: 'post',
                    data: {
                        action: 'camp_data_fetch',
                        keyword_search_doctors: jQuery('#contact-list-search').val()
                    },
