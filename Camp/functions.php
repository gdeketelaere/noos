...
//CSS	
	wp_enqueue_style('fonts-custom-style', get_stylesheet_directory_uri() . '/assets/css/fonts.css', 'fonts-style-css', 1);
	wp_enqueue_style('camp-custom-style', get_stylesheet_directory_uri() . '/assets/css/camp.css', 'doctreat-style-css', 2);
	wp_enqueue_style('noos-custom-style', get_stylesheet_directory_uri() . '/assets/css/noos.css', 'noos-style-css', 3);
...
function camp_delete_reference () {
	...
	if ( !is_admin() ) {
		...
		$comment  = (isset($_POST['camp_desc_reference_modified']) && !empty($_POST['camp_desc_reference_modified'])) ? $_POST['camp_desc_reference_modified'] : '' ;
		...
		if (!empty($ref_id) && !empty($post_id) && !empty($action) && $action === "modified") {

			//delete_post_meta( $post_id, '_saved_references');

			$ref=array('id'=>$ref_id,'comment'=>$comment);
			
			$_SESSION['camp_reference_modified'] = $ref;
			$_SESSION['camp_reference_modified_notice'] = 'yes';
...
// the ajax function
add_action('wp_ajax_camp_data_fetch_ref_new' , 'camp_data_fetch_ref_new');
add_action('wp_ajax_nopriv_camp_data_fetch_ref_new','camp_data_fetch_ref_new');

function camp_data_fetch_ref_new(){
	global $current_user;
	$user_identity 	 = $current_user->ID;
	$linked_profile  = doctreat_get_linked_profile_id($user_identity);
	$post_id 		 = $linked_profile;
	...
	$args = array(
		...
		's' 			    => esc_attr( $_POST['keyword_search_ref_new'] ),
	    'suppress_filters' 	=> false
	);
...
// the ajax function
add_action('wp_ajax_camp_data_fetch_ref_saved' , 'camp_data_fetch_ref_saved');
add_action('wp_ajax_nopriv_camp_data_fetch_ref_saved','camp_data_fetch_ref_saved');

function camp_data_fetch_ref_saved(){
	global $current_user;
	$user_identity 	 = $current_user->ID;
	$linked_profile  = doctreat_get_linked_profile_id($user_identity);
	$post_id 		 = $linked_profile;
	...
	$args = array(
		...
		's' 			    => esc_attr( $_POST['keyword_search_ref_saved'] ),
	    'suppress_filters' 	=> false
	);
...
function camp_share_linkedin(){
	$url=get_permalink();
	$linkedin='<a href="https://www.linkedin.com/shareArticle?mini=true&url='.$url.'" target="_blank"><i class="fab fa-linkedin-in" data-toggle="tooltip" data-placement="bottom" title="Linkedin"></i></a>';
	echo $linkedin;
